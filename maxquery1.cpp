...
{
    if ( low == high )
        seg[pos].insert({ar[low],low});
    else
    {
        ll mid = ( low + high ) >> 1;
        build ( low, mid, 2*pos + 1);
        build ( mid + 1, high, 2*pos + 2);
        for ( auto it : seg[2*pos+1] )
            seg[pos].insert(it);
        for ( auto it : seg[2*pos+2] )
            seg[pos].insert(it);
    }
}
ll query ( ll qlow, ll qhigh, ll low, ll high, ll pos, ll k )
{
    if ( qlow <= low and qhigh >= high )
        return seg[pos].size() - seg[pos].order_of_key({k,0});
    else if ( qhigh < low or qlow > high )
        return 0;
    else
    {
        ll mid = (low + high) >> 1;
        ll ql = query(qlow,qhigh,low,mid,2*pos+1,k);
        ll qr = query(qlow,qhigh,mid+1,high,2*pos+2,k);
        return ql+qr;
    }
}
int main()
