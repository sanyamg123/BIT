...
int main()
{
    ...
    {
        // offline processing
        cin >> queries[i].fi.fi >> queries[i].fi.sec;
        queries[i].sec = i;
    }
    // sorting the queries
    ...
    for ( ll i = 1; i <= q; i ++)
    {
        while ( las < queries[i].fi.fi )
        {
            // removing previous occurences by marking them 0
            update(las,0);
            idx[br[las]]++;
            if ( idx[br[las]] != pos[br[las]].size())
                update(pos[br[las]][idx[br[las]]],1);
            // marking next position with a 1
            las ++;
        }
        ans[queries[i].sec] = query(queries[i].fi.sec);// helps to store
        // answer to queries in a sorted manner
    }
    for ( ll i = 1; i <= q; i ++)
        cout << ans[i] <<"\n";
